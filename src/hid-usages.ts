...
import HidUsageNameZh from "./hid-usage-name-zh.json";
...
const zhNames: Record<string, Record<string, string>> = HidUsageNameZh;
...
export const hid_usage_get_zh_label = (
  usage_page: number,
  usage_id: number
): string | undefined =>
  zhNames[usage_page.toString()]?.[usage_id.toString()];

export const hid_usage_get_label = (
  usage_page: number,
  usage_id: number
): string | undefined =>
  hid_usage_get_zh_label(usage_page, usage_id) ||
...
export const hid_usage_get_labels = (
  ...
  const zh = hid_usage_get_zh_label(usage_page, usage_id);
  if (zh) {
    return { short: zh };
  }
